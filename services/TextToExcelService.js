...
module.exports = class TextToExcelService {
  ...
  init = () => {
    logger.log('info', `ID: ${this._socketId} init TextToExcelService , Target file : ${this._filename}`)
  ...
  start = async () => {
    ...
    const isServiceExist = await isExists('textToExcel')
    if (!isServiceExist) {
      let isSuccess = await signup('textToExcel')
    }
    const excelName = `${this._filename}_${outputFormat()}.xlsx`
    const parserProcess = new PythonShell('txt2excel_map.py', {
      mode: 'json',
    ...
    parserProcess.end((err, code, signal) => {
      if (err) {
        ...
        let add1 = usedService('textToExcel')
        this._emit({
          target: 'textToExcel',
