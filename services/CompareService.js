...
module.exports = class CompareService {
  ...
  init = () => {
    logger.log(
      'info',
      `ID: ${this._socketId} init CompareService , Target file : ${this._filename1} & ${this._filename2}`
    )
  ...
  start = async () => {
    ...
    const isServiceExist = await isExists('stdfCompare')
    if (!isServiceExist) {
      let isSuccess = await signup('stdfCompare')
    }
    const excelName = `Compare_Result_${outputFormat()}.xlsx`
    const parserProcess = new PythonShell('compare_STDF.py', {
      mode: 'json',
    ...
    parserProcess.end((err, code, signal) => {
      if (err) {
        ...
        let add1 = usedService('stdfCompare')
        this._emit({
          target: 'stdfCompare',
