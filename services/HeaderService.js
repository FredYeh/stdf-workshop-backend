...
module.exports = class HeaderService {
  ...
  init = () => {
    logger.log('info', `ID: ${this._socketId} init HeaderService , Target file : ${this._filename}`)
  ...
  start = async () => {
    ...
    const isServiceExist = await isExists('getHeader')
    if (!isServiceExist) {
      let isSuccess = await signup('getHeader')
    }
    const parserProcess = new PythonShell('getSTDFHeader.py', {
      mode: 'json',
    ...
    parserProcess.end((err, code, signal) => {
      if (err) {
        ...
        let add1 = usedService('getHeader')
        this._emit({
          target: 'stdfEditor',
          msg: { status: 'end', isFile: false }
        })
      }
    })
  }
